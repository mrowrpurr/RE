...
void Injected_DLL_Main() {
    ...
    //
    nana::form fm;
    fm.caption("Injected DLL");

    label lb1(fm, "LABEL 1");
    label lb2(fm, "LABEL 2");
    label lb3(fm, "LABEL 3");

    // label lb(fm, nana::rectangle(10, 10, 100, 20));
    // lb.caption("Hello, World!");

    // nana::button btn(fm, nana::rectangle(10, 40, 100, 20));
    // btn.caption("Quit");
    // btn.events().click([&fm] { fm.close(); });

    // Print("HELLO FROM THE INJECTED DLL!");
